...
const FrameLayers = memo(function FrameLayers() {
  return (
    <>
      ...
      {corners.map((c) => (
        // REFINED: Corner membesar, konsisten dengan section sebelumnya
        <div
          key={c.key}
...
const PhotoCard = memo(function PhotoCard({
  ...
  return (
    <m.button
      ...
      // FIX 1: Hapus `transition-all` yang bentrok dengan Framer Motion
      // FIX 2: Hapus `backdrop-blur-md` karena di-loop berkali-kali bikin GPU mobile menjerit
      // FIX 3: bg-white/80 diubah jadi bg-white/95 agar tetap solid & mewah
      className={`group relative aspect-[4/5] overflow-hidden rounded-[1.5rem] border border-mustard/30 bg-white/95 p-2 text-left shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-[border-color,box-shadow] duration-300 ease-out hover:border-mustard/60 hover:shadow-[0_15px_40px_rgba(212,175,55,0.15)] sm:rounded-[2rem] sm:p-3 ${
    ...
    >
      <div className="relative h-full w-full overflow-hidden rounded-xl border border-mustard/20 bg-blush/10">
        ...
        <Image
          ...
          // FIX 4: Animasi hover gambar diisolasi hanya pada transform & opacity
          className={`object-cover transition-[transform,opacity] duration-700 ease-out group-hover:scale-105 ${isLoaded ? "opacity-100" : "opacity-0"}`}
        />
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-t from-burgundy/70 via-ink/30 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <div className="flex translate-y-3 flex-col items-center gap-1.5 opacity-0 transition-[transform,opacity] duration-300 group-hover:translate-y-0 group-hover:opacity-100">
...
function Lightbox({
  ...
  useEffect(() => {
    ...
    // REFINED: Cleanup persisten untuk menghindari layout shift / nyangkut
            ...
            <div className="mt-2 flex items-center justify-center gap-2 overflow-x-auto px-2 pb-1 sm:mt-3 sm:gap-2.5">
              {photos.map((thumb, i) => (
                <button
                  ...
                  className={`relative h-11 w-11 shrink-0 overflow-hidden rounded-lg border transition-all duration-300 sm:h-14 sm:w-14 ${i === index ? "scale-105 border-burgundy opacity-100" : "border-mustard/30 opacity-50 hover:opacity-85"}`}
                >
                  <Image
                    src={thumb}
                    alt=""
...
const GALLERY_STYLES = `
  ...
  .animate-gallery-sway { animation: gallery-sway ease-in-out infinite; }

  @keyframes gallery-pulse {
    0%, 100% { transform: scale(1) rotate(0deg); }
    50%      { transform: scale(1.1) rotate(var(--rot, 5deg)); }
  }
  .animate-gallery-pulse { animation: gallery-pulse ease-in-out infinite; }

  @keyframes gallery-shimmer {
    0%   { transform: translateX(-100%); }
    100% { transform: translateX(100%); }
  }
  .animate-gallery-shimmer { animation: gallery-shimmer 1.6s ease-in-out infinite; }
  ...
  .animate-gallery-beam { animation: gallery-beam 17s ease-in-out infinite; }

  @media (prefers-reduced-motion: reduce) {
    .animate-gallery-sway, .animate-gallery-pulse, .animate-gallery-shimmer, .animate-gallery-beam { animation: none; }
  }
`;
...
function GallerySectionInner({ photos = DEFAULT_PHOTOS }: GallerySectionProps) {
  ...
  return (
    <section className="relative flex min-h-dvh w-full flex-col items-center justify-center overflow-hidden bg-[#FAF8F5] px-4 py-16 xs:px-5 sm:px-6 sm:py-24 md:py-28">
      ...
      {/* REFINED: AmbientLayer usang dibuang, diganti FloatingDecorations */}
      <FloatingDecorations />
      ...
      >
        ...
        >
          {photos.map((url, index) => (
            <PhotoCard key={url} url={url} index={index} onOpen={openAt} />
          ))}
        </m.div>
      </m.div>
